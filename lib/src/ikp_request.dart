...
class IkpRequest implements IcloudKitProvider {
  ...
  Future<IkpResponse<IkpRecord>> saveRecord({
    ...
    if (values.keys.isEmpty) {
      return IkpResponse(code: -401, msg: "values is empty");
    }
    ...
    if (errKeys.isNotEmpty) {
      msg += "fields [\"${errKeys.join("\", \"")}\"] is invalid;";
    }
    if (errValues.isNotEmpty) {
      msg += "values [${errValues.join(", ")}] is invalid;";
    }
    if (msg.isNotEmpty) {
    ...
    var result = await handler.call(
      ...
      params: {
        ...
        IkpConstants.recordValue: values,
      },
    );
    try {
  ...
  /// Gets records by [recordType] from CloudKit database
  @override
  ...
  /// Deletes the record by [recordName] from CloudKit database
  @override
  ...
  /// Clear CloudKit database by recordType
  @override
